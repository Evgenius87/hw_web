...
def folder_parser(path, semaphore):
    with semaphore:
...
def main(semaphore, path):
    
    make_correct_dirs(path)

    logging.basicConfig(level=logging.DEBUG, format='%(threadName)s %(message)s')
    semaphore = Semaphore(6)
    for adress, dirs, files in folder_parser(path, semaphore):
        thread = Thread(target=folder_parser, args=(path, semaphore))
        thread.start()
        thread.join()
        for file in files:
